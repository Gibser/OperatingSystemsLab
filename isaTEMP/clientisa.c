...
void PrintMatrix(){
	...
	for(int i=0;i<ROWS;i++){
		for(int j=0;j<COLS;j++){
			...
			if(game[i][j]=='o' || game[i][j]=='\0') printf("[ ]");
...
void play(){
	...
	char buffer[MAX], message[MAX], exit[MAX], mex[MAX];
	int nbytes=0;
	...
	while(1){ //menu di gioco
		...
		if(strcmp(buffer,"exit\n")==0){
			...
			else if(strcmp(mex,"nok")==0)printf("[+]Retry.\n");
			else if(strcmp(mex,"ok")!=0)printf("%s",mex);

			nbytes=read(sockfd,exit,MAX);
			exit[nbytes]='\0';
			if(strcmp(exit,"noendgame")!=0)break;
		}
	}
	sleep(1);
	system("clear");
	if(strcmp(buffer,"exit\n")!=0)printf("\t\t* * *%s* * *",exit); //stampo il vincitore solo se non sono uscito dal gioco
}
