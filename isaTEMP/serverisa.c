...
int OneUser(char buffer[MAX], int connection, int length){
	...
	for(int i=0;i<MAX_USERS;i++){
		if(OnlineUsers[i]){
			...
			if(strcmp(OnlineUsers[i]->username,user)==0){
				ret=1;
				break;
			}
		}
...
void InsertUser(int connection){
	...
	for(int i=0;i<ROWS;i++){
		for(int j=0;j<COLS;j++){
			index1=rand()%COLS;
			index2=rand()%COLS;
			if(count==0&&game[index1][index2]=='\0'){
				count++;
...
void MakeMatrix(){	
	...
	for(int i=0;i<ROWS;i++){
		for(int j=0;j<COLS;j++){
			game[i][j]='\0'; //inizializzo la matrice
		}
	}
...
void PressN(int connection){
	...
	if((index1+1)<ROWS){ // non esce dalla matrice
		if (game[index1+1][index2]=='\0'){ // non c'è un ostacolo
			pthread_mutex_lock(&MatrixMutex);
			game[index1][index2]='\0';
			...
			nbytes=sprintf(buffer,"ostacolo");
			write(connection,buffer,MAX);
		}
		else if(game[index1+1][index2]!='x'&&game[index1+1][index2]!='+'&&game[index1+1][index2]!='o'&&game[index1+1][index2]!='-'&& game[index1+1][index2]!='\0'){ // scontro con un altro giocatore
			nbytes=sprintf(buffer,"scontro");
			write(connection,buffer,MAX);
		}
		else if(game[index1+1][index2]=='+'){ // pacco
			if(OnlineUsers[indexUser]->packages==0){
				pthread_mutex_lock(&MatrixMutex);
				game[index1+1][index2]='\0';
				pthread_mutex_unlock(&MatrixMutex);
				pthread_mutex_lock(&ClientMutex);
			...
			if(OnlineUsers[indexUser]->packages!=0){
				if(FindPack(SearchPack(connection,1),SearchPack(connection,2))==1){
					...
					pthread_mutex_unlock(&ClientMutex);
					pack--;
					nbytes=sprintf(buffer,"consegnato");
					write(connection,buffer,MAX);
				}
				else{
					OnlineUsers[indexUser]->packages--;
					pack--;
					nbytes=sprintf(buffer,"perso");
					write(connection,buffer,MAX);
				}
			}
			else{
				nbytes=sprintf(buffer,"nopacchi");
				write(connection,buffer,MAX);
			}
		}
	}
...
void PressS(int connection){
	...
	if((index2-1)>=0){ // non esce dalla matrice
		if (game[index1][index2-1]=='\0'){ // non c'è un ostacolo
			pthread_mutex_lock(&MatrixMutex);
			game[index1][index2]='\0';
		...
		else if(game[index1][index2-1]!='x'&&game[index1][index2-1]!='+'&&game[index1][index2-1]!='o'&&game[index1][index2-1]!='-'&& game[index1][index2-1]!='\0'){ // scontro con un altro giocatore
			nbytes=sprintf(buffer,"scontro");
			write(connection,buffer,MAX);
		}
